(function () {
  ...
  async function tryAutoLogin() {
    ...
    try {
      const res = await API.authenticate();
      if (res && res.user_info && String(res.user_info.auth) === '1') { enterApp(); return true; }
    } catch (e) {}
    return false;
  }
  ...
  async function loadAllContent() {
    ...
    if (el) el.insertAdjacentHTML('afterbegin', '<div id="loading-content" class="empty">Carregando conteúdo do servidor...</div>');
    try {
    ...
    finally { const l = $('#loading-content'); if (l) l.remove(); switchView(state.view); }
  }
  ...
  function playLive(id, title) { Player.open(API.liveUrl(id), title || 'Canal ao vivo'); }
  ...
  async function playVod(id, title) {
    ...
    Player.open(API.vodUrl(id, ext), title || 'Filme');
  }
